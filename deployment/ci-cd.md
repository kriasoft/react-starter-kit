...
Migrations run *before* the new workers, so the currently deployed workers keep serving traffic against the migrated schema. A rollback replaces workers but does not revert the schema.
...
The two environments exist for ordering. Protection rules gate *any* job naming an environment, so a single environment would put the approval before the plan. The `-plan` environment therefore carries the branch rule but no reviewers, and sets `deployment: false` so planning does not record a deployment. It also holds the plan-only token, so the credential available before the approval cannot apply.
...
Use `--env staging`, not the top-level config. The top level *is* production, so a preview of it would carry production service bindings and a publicly reachable URL would serve production data.
