# Staging environment: the shared target for app deploys from main.
#
...
output "hyperdrive_cached_id" {
  ...
  # Hard-coding `environment` protects resource names, not state. TF_WORKSPACE
  # selects state, and an exported value overrides the local env file; require
  # this suffix so the staging root cannot mutate production state.
  precondition {
    condition     = endswith(terraform.workspace, "-staging")
    error_message = "Refusing to run: the staging root requires an HCP workspace name ending in \"-staging\", but TF_WORKSPACE resolved to \"${terraform.workspace}\". Check for an exported TF_WORKSPACE overriding .env.terraform.staging.local."
  }
}
