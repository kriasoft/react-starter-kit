...
- Primary keys use application-generated prefixed CUID2 IDs: `generateAuthId(model)` for Better Auth models and `generateId("xxx")` for domain tables. See `schema/id.ts` for the prefix map.
...
- `member.role` ("owner", "admin", "member") and `invitation.status` are free `text`, not pgEnum – avoids fragile coupling with Better Auth's values.
- `organization.metadata` is `text`, not JSONB – Better Auth handles serialization.
- Migrations differ by role. A project generated from this kit keeps ordinary migration history – see `docs/database/migrations.md`. The starter kit itself squashes into a single `0000_init.sql`; contributors changing its schema should read `.github/CONTRIBUTING.md` first.
...
- Every foreign key column gets an index: `{table}_{column}_idx`. All foreign keys use `onDelete: "cascade"`.
- Composite uniques: `member(userId, organizationId)`, `invitation(organizationId, email)`, `identity(providerId, accountId)`.
- Better Auth keys an identity on `(providerId, accountId)` and throws when a lookup matches two rows. Never add an `issuer` column back – see the note on `identity` in `schema/user.ts`.
- The `invitation` unique is a starter limitation: Better Auth creates a new row for a later invitation, so remove the constraint before supporting re-invites after acceptance, rejection, or cancellation.
...
- It returns `{ db, reset, close }`. One instance at module scope (`await createTestDatabase()`), `reset()` between tests, `close()` in `afterAll`. `reset()` truncates every table in the schema, derived from the barrel, so a new table is covered automatically.
